# ============================================================
# LLM_verb_comparison_xlsx.py
# ============================================================
# Uses an LLM to extract EN→DE verb pairs from bilingual patent segments,
# then flags inconsistencies purely from the canonical glossary — no
# evaluator LLM pass.
#
# INPUT   projects/<project_id>/*_translated.xlsx
# OUTPUT  projects/<project_id>/verb_segment_pairs.csv
#                               verb_canonical_glossary.csv
#                               verb_flags.csv
#                               <name>_checks.xlsx
# ============================================================
...
# ============================================================
# PHASE 1 — Extract verb pairs per segment via batched LLM calls
# ============================================================
...
def build_batch_prompt(segments):
    ...
    return (
        ...
        "For each numbered segment pair, identify the English content verbs in the EN text "
        "and find their German translation equivalents in the DE text.\n\n"
        "Patent-specific rule: German patent claims use nominalized infinitives as verbal nouns "
        "(e.g. 'das Empfangen von Daten' = 'the receiving of data'). "
        "These are verbal in meaning. Recognize them and map them to the English source verb "
        "(e.g. Empfangen → receive, Übertragen → transmit, Zuordnen → associate).\n\n"
        "Return ONLY a JSON array — no markdown, no explanation. Each element:\n"
        '  {"id": "<segment_id>", "pairs": [{"en": "<infinitive>", "de": "<infinitive>"}, ...]}\n\n'
        "Rules:\n"
        "  - Use base/infinitive forms only (receive not received; empfangen not empfängt)\n"
        "  - Include only verbs that have a clear translation counterpart in the other language\n"
        "  - Skip generic auxiliaries: be, have, become, sein, haben, werden, können, müssen\n"
        "  - If a segment has no clear EN-DE verb pair, return an empty pairs array\n"
...
print(f"\nPhase 1: extracting verb pairs — {len(batches)} batches of up to {BATCH_SIZE} segments...")
...
print(f"\nTotal verb pairs extracted across all segments: {len(all_pairs)}")

if not all_pairs:
    print("No verb pairs extracted. Cannot continue. Check the API connection and model response above.")
    exit(1)
...
pairs_csv = str(proj_dir / "verb_segment_pairs.csv")
...
print('Saved "verb_segment_pairs.csv".')
...
glossary_df.to_csv(str(proj_dir / "verb_canonical_glossary.csv"), index=False, encoding="utf-8-sig")
print('\nSaved "verb_canonical_glossary.csv".')

print("\nCanonical glossary (most common translation per EN verb):")
...
print("\nPhase 3 – Case 1: checking for inconsistent verb translations...")
case1_count = 0
...
for en_verb, de_counter in en_to_de.items():
    ...
    for p in all_pairs:
        ...
        if clear_winner:
            note = (
                f"[Case 1] {en_verb}: minority translation \"{p['de_verb']}\" "
                f"(canonical: {canonical_de} {canonical_count}/{total}; also seen: {alts})"
            )
        else:
            note = (
                f"[Case 1] {en_verb}: no dominant translation "
                f"({', '.join(f'{d}({c})' for d, c in de_counter.most_common())})"
            )

        flags.append({
            "Segment ID":  p["segment_id"],
            "Flag Type":   "Case 1 – minority" if clear_winner else "Case 1 – no winner",
            "Note":        note,
...
print("Phase 3 – Case 2: checking for same DE verb used for multiple EN verbs...")
case2_count = 0
...
for p in all_pairs:
    ...
    note = (
        f"[Case 2] \"{p['de_verb']}\": same translation for different source verbs: "
        + ", ".join(en_sources)
    )
    flags.append({
        "Segment ID":  p["segment_id"],
        "Flag Type":   "Case 2 – shared DE verb",
        "Note":        note,
        "Source Text": p["source_text"],
        "Target Text": p["target_text"],
    })
...
flags_df.to_csv(str(proj_dir / "verb_flags.csv"), index=False, encoding="utf-8-sig")
print('Saved "verb_flags.csv".')
...
# Find existing "Verb Flags" column or insert one at position 4
verb_col = next(
    (c for c in range(1, ws.max_column + 1) if ws.cell(row=1, column=c).value == "Verb Flags"),
    None
)
if verb_col is None:
    ws.insert_cols(4)
    verb_col = 4
    ws.cell(row=1, column=verb_col).value = "Verb Flags"

for row_num in range(4, ws.max_row + 1):
    seg_id = str(ws.cell(row=row_num, column=1).value)
    ws.cell(row=row_num, column=verb_col).value = (
        "\n".join(seg_annotations[seg_id]) if seg_id in seg_annotations else None
    )
    ws.cell(row=row_num, column=verb_col).alignment = openpyxl.styles.Alignment(wrap_text=True)
try:
    wb.save(out_path)
