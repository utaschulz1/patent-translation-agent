"""glossary_lib.lemma_sync — auto-growth of the verb lemma lookup tables.

Moved from verb_lemma_sync.py; see that module's original header for the full
history (the "having" false positive of 2026-07-31 this exists to prevent).
...
from glossary_lib.validate import parse_json_object_lenient

AGENT_DIR = Path(__file__).parent.parent

EN_LEMMA_PATH = AGENT_DIR / "EN_verb_lemma_lookup.json"
DE_LEMMA_PATH = AGENT_DIR / "DE_verb_lemma_lookup.json"

# Same suffixes glossary_lib.matching's _count_lemmas() strips at check time
# when a DE word isn't found directly (Partizip-II adjective inflection,
# e.g. "eingerichtete" -> "eingerichtet"). A form the checker can already
# resolve this way counts as "known" here too — we don't need a table entry
# for every adjective ending of every participle.
...
def write_lemma_table(path: Path, table: dict[str, str]) -> None:
    """Write a lemma table as sorted, indented JSON with a trailing newline."""
