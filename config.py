...
Optional .env key:
    WORK_DIR   Absolute path to the job storage root. Defaults to agent/projects/.
               Windows+OneDrive:  C:\\Users\\you\\OneDrive\\WORKFOLDER\\Client Name
               Linux+GDrive:      /home/you/GoogleDrive/WORKFOLDER/Client Name
...
# No real name hardcoded here (this file is in the public repo) — set the
# real value in .env, which is gitignored.
TRANSLATOR_NAME = os.environ.get("TRANSLATOR_NAME", "Translator")

# Client code (2-6 uppercase letters) + YYMM + "P" + job number, e.g.
# "ACME_2606_P1200". XTRF job type prefixes/suffixes around this core ID
# (e.g. "MT Light of ACME_2606_P1200", "ACME_2606_P0022 Issues resolution")
...
def extract_project_id(project_name: str) -> str:
    ...
    XTRF projectName is e.g. "Patents | ACME_2604_P0732" or, for Light
    Post-editing / Issues resolution jobs, "Patents | MT Light of
    ACME_2606_P1200". Only the core ID matters elsewhere (XTM task lookup,
...
def is_issue_resolution_job(overview: dict) -> bool:
    ...
    Resolution" and, confirmed against a real live job, "Hourly tasks" (case
    varies on both). A keyword match on the type field alone would have
    silently misrouted that job through the normal post-editing path instead
    of the Issue Resolution one. The one constant across every label variant
    seen so far is that these jobs are priced at 0 — that job's
